...
public class ExcelReaderNew {
     ...
     * @throws Exception
     */
    public ExcelReaderNew(String filepath) throws Exception {
        InputStream is = new FileInputStream(filepath);
        wb = WorkbookFactory.create(is);
        sheet = wb.getSheetAt(0);
    }
    
    public ExcelReaderNew(InputStream is)  throws Exception {
        wb = WorkbookFactory.create(is);
        sheet = wb.getSheetAt(0);
    }
    
    public ExcelReaderNew(String filepath,int sheetNo) throws Exception {
        InputStream is = new FileInputStream(filepath);
        wb = WorkbookFactory.create(is);
        sheet = wb.getSheetAt(sheetNo);
    }
    
    public ExcelReaderNew(InputStream is,int sheetNo)  throws Exception {
        wb = WorkbookFactory.create(is);
        sheet = wb.getSheetAt(sheetNo);
    }
