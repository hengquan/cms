package com.spirit.core.dict.service;
...
import com.spirit.core.dict.model.DictMaster;
import com.spirit.core.dict.model.DictModel;
import com.spirit.core.dict.model.DictRefRes;
import com.spirit.core.dict.persis.po.DictDetailPo;
import com.spirit.core.dict.persis.po.DictMasterPo;
...
//                }
            ...
            throw new RuntimeException("加载Session中的字典信息", e);
        }
    }
    /*
    ...
    public void addDictDetail(DictDetail dd) {
        try {
            ...
            throw new RuntimeException("新增字典项", e);
        }
    }
    ...
    public int bindDictRef(DictRefRes drr) {
        try {
            CacheEle<_CacheDictionary> cache=((CacheEle<_CacheDictionary>)SystemCache.getCache("CACHE_DICT"));
            _CacheDictionary cd=cache.getContent();
            DictRefResPo newDrrPo=drr.convert2Po();
            DictModel dm=cd.dictModelMap.get(newDrrPo.getDmId());
            if (dm==null||dm.dictTree==null) return 2;
            TreeNode<DictDetail> dd=(TreeNode<DictDetail>)dm.dictTree.findNode(newDrrPo.getDdId());
            if (dd==null) return 3;

            Map<String, Object> param=new HashMap<String, Object>();
            param.put("tabName", drr.getTabName());
            param.put("tabId", drr.getTabId());
            List<DictRefRes> drrl=this.getDictRefs(param);
            boolean exist=false;
            if (drrl!=null&&!drrl.isEmpty()) {
                ...
                for (DictRefRes _drr: drrl) {
                    if (_drr.getTabId()!=null&&_drr.getTabId().equals(drr.getTabId())
                      &&_drr.getTabName()!=null&&_drr.getTabName().equals(drr.getTabName())
    ...
    public List<DictRefRes> getDictRefs(Map<String, Object> condition) {
        ...
        try {
            ...
            for (DictRefResPo drrPo: l) {
                ...
                DictModel dm=cd.dictModelMap.get(drrPo.getDmId());
                drr.setDm(dm);
                if (dm.dictTree!=null&&dm.dictTree.getChildren()!=null&&!dm.dictTree.getChildren().isEmpty()) {
                    TreeNode<TreeNodeBean> dd=(TreeNode<TreeNodeBean>)dm.dictTree.findNode(drrPo.getDdId());
