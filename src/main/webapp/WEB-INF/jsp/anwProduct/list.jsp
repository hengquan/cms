		...
		<section id="main-content">
			<section class="wrapper">
				...
				<div class="row">
					<div class="col-lg-12">
						<section class="panel">
							<header class="panel-heading">商品信息管理</header>
							...
							<table class="table table-striped border-top" id="sample_1">
								<thead>
									<tr>
										...
										<th class="hidden-phone">商品规格</th>
										<th class="hidden-phone">商品图片</th>
										<th class="hidden-phone">商品价格</th>
										<th class="hidden-phone">含量(袋)</th>
									...
									<c:forEach items="${anwProducts}" var="u" varStatus="s">
										<tr class="odd gradeX theTr">
											<td><input type="checkbox" name="box" class="checkboxes"
												value="${u.id}" /></td>
											<td class="hidden-phone">${u.productName}</td>
											<td class="hidden-phone">${u.standardName}</td>
											<td class="hidden-phone"><img alt="" src="${appAccessUrl}${u.img}" height="50" > </td>
											<td class="hidden-phone">${u.price}</td>
											<td class="hidden-phone">${u.unit}</td>
											<td class="hidden-phone">${u.salesVolume}</td>
											<td class="hidden-phone">${u.freight}</td>
											<td class="hidden-phone">
												<input type="number" class="form-control" value="${u.isIndex}" style="width:100px;" id="${u.id}" onBlur="updateIndexValue(this)" ">
											</td>
											<td>
												<button type="button" class="btn btn-primary" onclick="doEdit('${u.id}')">查看商品详情</button>
												<button type="button" class="btn btn-primary" onclick="seeProduct('${u.id}')">查看商品评论信息</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
