						...
						<section class="panel">
							...
							<div class="modal-body">
								<form class="form-horizontal" role="form" id="itemForm"
									...
									<c:if test="${!empty answerMessage.answerContentText}">
										<div class="form-group">
											<label class="col-lg-1 control-label pd-r5">文字消息<font
												style="color: red;"></font></label>
											<div class="col-lg-6">
												<textarea rows="5" cols="60" class="form-control">${answerMessage.answerContentText}</textarea>
											</div>
										</div>
									</c:if>
							...
							<table class="table table-striped border-top" id="sample_1">
								<thead>
									<tr>
										...
										<th class="hidden-phone">是否为幸运偷听者</th>
										<th class="hidden-phone">创建时间</th>
									</tr>
								</thead>
								<tbody id="theTbody">
									<c:forEach items="${eavesdrops}" var="u" varStatus="s">
										<tr class="odd gradeX theTr">
											...
											<td class="hidden-phone">
												<c:if test="${u.is_good_fortune==0}">否</c:if>
												<c:if test="${u.is_good_fortune==1}">是</c:if>
											</td>
					...
					<form class="form-horizontal" role="form" id="itemForm"
						...
						<div class="form-group" id="phoneMessge">
							<label class="col-lg-2 control-label pd-r5">语音消息<font
								style="color: red;"></font></label>
							<div class="col-lg-10">
								<audio src="${appRoot}/static/video/123.mp3" controls="controls" id="phoneContent">
								</audio>
							</div>
						</div>
		...
		function problemMessage(contentText,id){
			...
			$.ajax({
				...
				success:function(data){
					if(data.msg==0){
						$("#textContent").val(data.answer.answer_content_text);
						$(".titleMessage").html("回答信息");
						$("#videoMessge").show();
						if(data.answer.answer_content_video!=undefined){
							$("#videoContent").prop("src",data.answer.answer_content_video);
							$("#videoMessge").show();
						}
						//取语音和图片信息
