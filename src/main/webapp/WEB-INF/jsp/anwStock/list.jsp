		...
		<section id="main-content">
			<section class="wrapper">
				...
				<div class="row">
					<div class="col-lg-12">
						<section class="panel">
							<header class="panel-heading">库存管理</header>
							<form action="${appRoot}/anwStock/list" method="post"
								...
								<thead>
									<tr>
										<th style="width: 8px;"><input type="checkbox" name="box"
											...
											value="" /></th>
										<th class="hidden-phone">库存管理员</th>
										<th class="hidden-phone">送货人姓名</th>
										<th class="hidden-phone">送货人电话</th>
										<th class="hidden-phone">商品名称</th>
										<th class="hidden-phone">站点</th>
										<th class="hidden-phone">库存量(应入库)</th>
										<th class="hidden-phone">库存量(已入库)</th>
										<th class="hidden-phone">货运状态</th>
										<th class="hidden-phone">进库时间</th>
									...
									<c:forEach items="${userList}" var="u" varStatus="s">
										<tr class="odd gradeX theTr">
											<td><input type="checkbox" name="box" class="checkboxes"
												value="${u.id}" /></td>
											<td class="hidden-phone">${u.user_name}</td>
											<td class="hidden-phone">${u.delivery_man}</td>
											<td class="hidden-phone">${u.mobile}</td>
											<td class="hidden-phone">${u.product_name}    ${u.standardName }</td>
											<td class="hidden-phone">${u.name}</td>
											<td class="hidden-phone">${u.unreceived_stock}</td>
											<td class="hidden-phone">${u.stock}</td>
											<td class="hidden-phone">
												<c:if test="${u.state == 0}">待入库</c:if>
												<c:if test="${u.state == 1}">已入库</c:if>
											</td>
											<td class="hidden-phone">${u.update_time}</td>
											<td><button type="button" class="btn btn-primary"
													onclick="doEdit('${u.id}','${u.user_name}','${u.product_id}','${u.storeroom_id}','${u.stock}','${u.unreceived_stock }')">修改</button>
												<button type="button" class="btn btn-primary"
													onclick="seeCause('${u.cause}')">查看原因</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
				...
				<div class="modal-body">
					<form action="${appRoot}/anwStock/add" method="post"
						...
						<div class="form-group">
							...
							<div class="col-lg-10">
								<select class="form-control" name="storeroomId">
							...
							<div class="col-lg-10">
								<select class="form-control" name="productId">
									<c:forEach items="${anwProducts}" var="u" varStatus="s">
										<option value="${u.id }">${u.productName }(${u.standardName })</option>
									</c:forEach>
								</select>
							</div>
						</div>
						...
						<div class="form-group">
							<label class="col-lg-2 control-label pd-r5">应入库数量<font
								style="color: red;"></font></label>
							<div class="col-lg-10">
								<input type="number" class="form-control" name="unreceivedStock" maxlength="10">
							</div>
						</div>
						<div class="form-group">
					...
					<form action="${appRoot}/anwStock/edit" method="post"
						...
						<div class="form-group">
							<label class="col-lg-2 control-label pd-r5">应入库数量<font
								style="color: red;"></font></label>
							<div class="col-lg-10">
								<input type="number" class="form-control" name="unreceivedStock" id="unreceivedStock1" maxlength="10">
							</div>
						</div>
						<div class="form-group">
							<div class="col-lg-offset-2 col-lg-10">
