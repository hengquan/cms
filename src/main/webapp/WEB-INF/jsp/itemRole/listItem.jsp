...
<body>
              ...
              <div class="col-lg-12">
                <div class="vga-wrapper">
                  <section class="panel vga-tit">
                      ...
                      <div class="value">
                          <button id="submitBtn" type="button" class="btn btn-success" style="padding-left:30px;padding-right:30px;">保 存</button> 
                  ...
                  <div class="col-lg-12" style="overflow-y:auto;">
                      <section class="panel">
                          ...
                          <div class="panel-body">
                              <form class="form-horizontal" role="form" id="baseForm" name="baseForm" action="${appRoot}/roleItem/addAndUpdate" method="post">
                            	<input type="hidden" name="roleId" value="${roleId }">
                            	<input type="hidden" name="itemId" value="${itemId }">
                            	<input type="hidden" name="id" value="${id }">
                            	<input type="hidden" name="itemIds" id="itemIds" value="">
                                  <div class="form-group">
                                      <label class="col-lg-3 control-label pd-r5">角色名称<font style="color:red;">*</font></label>
                                      <div class="col-lg-11">
                                          <input type="text" class="form-control" id="versionNum" name="versionNum" placeholder="" value="${roleName }" readonly="readonly">
                                      </div>
                                  </div>
                                  <div class="form-group">
                                      <label class="col-lg-3 control-label pd-r5">角色权限<font style="color:red;">*</font></label>
                                      
                                      <div class="col-lg-11" id="cbox">
                                      <input type="checkbox" name="Fruit1" id="chkAll" class="group-checkable" onclick="checkEvent('Fruit','chkAll')" />全选
                                      ...
                                      <c:set value="1" var="num"></c:set>
                                      		<div class="${item.id }">
                                        	<label onclick="cke(this)" data-id="${num}">
                                        	<input name="Fruit" type="checkbox" value="${item.id}" 
                                        	<c:forEach items="${list}" var="info" varStatus="e">
                                        		<c:if test="${item.itemName == info.itemName }">checked="true"</c:if>
                                        	</c:forEach>/>
                                        	${item.itemName} 
                                        	</label>
                                        	<c:forEach items="${item.childList}" var="childItem" varStatus="e">
                                        		<c:set value="${num+1}" var="num"></c:set>
                                        		<label data-id="${num}">
                                        		<input name="Fruit" class="sys-item" type="checkbox" value="${childItem.id}" 
    ...
    <script>
      ...
      $('.sys-item').click(function(){
    	  var ck = $(this).parent();
    	  var len = $(this).prop('checked')?1:0;
    	  $(this).parent().siblings().each(function(){
    		  if($(this).attr('data-id')!= 1){
    			  len+= $(this).find('input').prop('checked')?1:0;
    		  }
    	  });
    	  $(this).parent().siblings('[data-id="1"]').find('input').prop('checked',len > 0?true:false);
    	  console.log(len);
      });
    </script>
