...
<body>
				...
				<div class="row">
					<div class="col-lg-12">
						<section class="panel">
							...
							<form action="${appRoot}/anwOrder/list" method="post"
								...
								<div
									...
									<input type="text" class="btn"
										...
										placeholder="请输入用户姓名，昵称或订单号" name="userName1" value="${mingzi }"><span>
								...
								<thead>
									<tr>
										<th style="width: 8px;"><input type="checkbox" name="box"
											...
											value="" /></th>
										<th class="hidden-phone">快递单号</th>
										<th class="hidden-phone">买家姓名</th>
										<th class="hidden-phone">订单总金额</th>
										<th class="hidden-phone">优惠券金额</th>
										<th class="hidden-phone">红包金额</th>
										<th class="hidden-phone">会员折扣金额</th>
										<th class="hidden-phone">积分金额</th>
										<th class="hidden-phone">订单状态</th>
										<th class="hidden-phone">购买时间</th>
									...
									<c:forEach items="${selectList}" var="u" varStatus="s">
										<tr class="odd gradeX theTr">
											<td><input type="checkbox" name="box" class="checkboxes"
												value="${u.id}" /></td>
											<td class="hidden-phone">${u.deliveryNumber}</td>
											<td class="hidden-phone">${u.name}</td>
											<td class="hidden-phone">${u.totalAmount}</td>
											<td class="hidden-phone">${u.couponAmount}</td>
											<td class="hidden-phone">${u.bonusAmount}</td>
											<td class="hidden-phone">${u.discounAmount}</td>
											<td class="hidden-phone">${u.integralAmount}</td>
											<td class="hidden-phone">
												<c:if test="${u.orderState == 0}">未支付</c:if>
												<c:if test="${u.orderState == 1}">已支付</c:if>
												<c:if test="${u.orderState == 2}">已发货</c:if>
												<c:if test="${u.orderState == 3}">已签收</c:if>
												<c:if test="${u.orderState == 4}">已评价</c:if>
												<c:if test="${u.orderState == 5}">取消</c:if>
												<c:if test="${u.orderState == 6}">申请退款</c:if>
												<c:if test="${u.orderState == 7}">退款成功</c:if>
											</td>
											<td class="hidden-phone">
												<fmt:formatDate value="${u.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
											</td>
											<td><button type="button" class="btn btn-primary"
													onclick="seeMsg('${u.id}')">查看订单详情</button></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="modal-title">查看购买物品</h4>
				</div>
				...
				<div class="modal-body">
					<form action="${appRoot}/anwStock/edit" method="post"
						...
						<div class="form-group">
							<label class="col-lg-2 control-label pd-r5">站点<font
						...
						<div class="form-group">
							<label class="col-lg-2 control-label pd-r5">商品名称<font
								style="color: red;"></font></label>
							<div class="col-lg-10">
								<select class="form-control doEditSelect2" name="productId">
									<c:forEach items="${anwProducts}" var="u" varStatus="s">
										<option value="${u.id }">${u.productName }</option>
									</c:forEach>
								</select>
							</div>
						</div>
						<div class="form-group">
							<label class="col-lg-2 control-label pd-r5">送货人姓名<font
								style="color: red;"></font></label>
							<div class="col-lg-10">
								<input type="text" class="form-control" name="deliveryMan" id="deliveryMan1" maxlength="10">
							</div>
						</div>
						<div class="form-group">
							<label class="col-lg-2 control-label pd-r5">电话<font
								style="color: red;"></font></label>
							<div class="col-lg-10">
								<input type="text" class="form-control" name="mobile" id="mobile1" maxlength="10">
							</div>
						</div>
						<div class="form-group">
							<label class="col-lg-2 control-label pd-r5">库存量<font
								style="color: red;"></font></label>
							<div class="col-lg-10">
								<input type="number" class="form-control" name="stock" id="stock1" maxlength="10">
							</div>
						</div>
						<div class="form-group">
	...
	<script type="text/javascript">
		...
		function submitUpdateData() {
			var deliveryMan = $("#deliveryMan1").val();
			if (deliveryMan == '' || deliveryMan == undefined) {
		...
		//修改菜单
		function doEdit(id,userName,deliveryMan,mobile,productId,storeroomId,stock) {
		...
		//查看信息
		function seeMsg(id){
			window.location.href="${appRoot}/anwOrder/seemsg?id="+id;
			
			
			/* $.ajax({
				type:'post',
				data : {"id":id},  
				url:'${appRoot}/anwOrder/seemsg',
				dataType:'json',
				success:function(data){
					if(data.msg==0){
						var content = data.anwOrderProduct;
						var orderMsg ='<input type="hidden" name="editId1" id="editId1">';
						$.each(content,function(i,item){
							 orderMsg += '<div class="form-group">'+
								'<label class="col-lg-2 control-label pd-r5">商品名称<font'+
									'style="color: red;"></font></label>'+
								'<div class="col-lg-2">'+
									'<input type="text" class="form-control" value="'+ item.name +'" />'+
								'</div>'+
								'<label class="col-lg-2 control-label pd-r5">商品价格<font'+
									'style="color: red;"></font></label>'+
								'<div class="col-lg-2">'+
									'<input type="text" class="form-control" value = "'+ item.price +'"/>'+
								'</div>'+
								'<label class="col-lg-2 control-label pd-r5">购买数量<font'+
									'style="color: red;"></font></label>'+
								'<div class="col-lg-2">'+
									'<input type="text" class="form-control" value="'+ item.product_number +'" />'+
								'</div>'+
							'</div>'
						});
						$("#addMessage").html(orderMsg);
						var $modal = $('#addProblemType');
						$modal.modal();
					}else{
						windowShow("操作失败","");
