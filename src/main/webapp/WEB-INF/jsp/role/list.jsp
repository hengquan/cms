...
<body>
		...
		<section id="main-content">
			<section class="wrapper">
				...
				<div class="row">
					<div class="col-lg-12">
						<section class="panel">
							<header class="panel-heading">系统管理--角色信息管理 </header>
								...
								<tbody>
									<c:forEach items="${roleList}" var="info" varStatus="s">
										<tr class="odd gradeX">
											...
											<td class="hidden-phone">${info.logogram}</td>
										<%-- 	<td class="hidden-phone"><fmt:formatDate value="" type="both" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%><%-- ${info.createTime} --%>
											<td class="hidden-phone">
												<button class="btn btn-primary btn-xs" onclick="doEdit('${info.id}','${info.roleName}','${info.pinyin}','${info.logogram}','${info.remark}')">
													<i class="icon-pencil"></i>
												</button> &nbsp;
												<button class="btn btn-danger btn-xs" onclick="doDel('${info.id}')">
													<i class="icon-trash"></i>
												</button> &nbsp;
												<a href="${appRoot}/roleItem/listItem?roleId=${info.id}&roleName=${info.roleName}&itemId=${itemId}&id=${id}">
												<button class="btn btn-danger btn-xs"><i class="icon-eye-open">查看角色权限</i></button>
												</a>
											</td>
										</tr>
									</c:forEach>
					...
					<div class="modal-body">
						<form class="form-horizontal" role="form" id="roleForm" name="roleForm" >
							...
							<div class="form-group">
								<label class="col-lg-2 control-label pd-r5">角色名称<font style="color:red;">*</font></label>
								<div class="col-lg-10">
							...
							<div class="form-group">
								<label class="col-lg-2 control-label pd-r5">角色拼音<font style="color:red;">*</font></label>
								<div class="col-lg-10">
							...
							<div class="form-group">
								<label class="col-lg-2 control-label">角色简拼<font style="color:red;">*</font></label>
								<div class="col-lg-10">
									<input type="text" class="form-control" id="logogram" name="logogram" placeholder="请输入角色简拼">
								</div>
							</div>
							 <div class="form-group">
	...
	<form action="${appRoot}/role/del" method="post" id="deleForm" name="deleForm">
...
/* 	function validateForm(){
		...
		if($('#logogram').val() == null || $('#logogram').val() == ''){
			windowShow("角色简音不能为空","");
			return false;
		}
	}
	 */
		...
		function doEdit(id,roleName,pinyin,logogram,remark) {
			$('#editId').val(id);
			$('#roleName').val(roleName);
			$('#pinyin').val(pinyin);
			$('#logogram').val(logogram);
			$('#remark').val(remark);
			var $modal = $('#myModal');
